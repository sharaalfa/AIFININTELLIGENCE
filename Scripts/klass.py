...
def ml(y,a):# a- как правило, 0.3,0.4-соотношение обучающей
    # тестовой выборок
    ...
    try:
        ...
        testModels1.to_excel('D:/work/data1/aaaa.xlsx')
        ...
        testModels=testModels.append([tmp1,tmp2,tmp3,tmp4])
        fig,axes=plt.subplots(ncols=2,figsize=(10,4))
        testModels.plot(ax=axes[0],kind='bar',title='neo',x='Model')
        #testModels.plot(ax=axes[1],kind='bar',color='green',title='R2_Y2')

        return testModels,show()
