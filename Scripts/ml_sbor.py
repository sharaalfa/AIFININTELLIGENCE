import pandas as pd
import chmath as rol
import chmath as vol
import chmath as rprp
...
from pylab import *
#казахстан
url_kazah = 'http://www.nationalbank.kz/cont/stand_12m2015%20(report30.03.16)_rus.xls'
#россия
...
data_usa = pd.read_excel('D:/work/data1/USA1a.xlsx', skeep_footer=6, index_col=0)
data_kz = pd.read_excel(url_kazah, header=2, index_col=0,
                        parse_cols="A,P,R:U,W:Z,AB:AE,AG:AJ,AL:AO,AQ:AT,AV:AY,BA:BD")

data_ch=pd.read_excel('D:/work/data1/maych1a.xlsx', index_col=0, parse_cols="A:AH")
...
data_15 = pd.read_excel(url_15, header=3, parse_cols="A:E",skip_footer=6, index_col=0)
data_14_A = pd.read_excel(url_14_A, header=3, parse_cols="A:E",skip_footer=6, index_col=0)
data_15_A = pd.read_excel(url_15_A, header=3, parse_cols="A:E",skip_footer=6, index_col=0)
...
kaz_neo = kz_neo.unstack().head(33)
thusa_neo= usa_neo.unstack().head(33)

china_neo=ch_neo.unstack().head(33)

brit=dfdf.dfdf('D:/work/data/brit1.xlsx',"A12:A44").unstack().head(33)
...
def kv2():
    kv2 = pd.DataFrame({'Чистые ошибки и пропуски(С)':kv_11,
                    ...
                    'Абс. чоп':abs(kv_23),
                    #'Индекс сред.':rol.rol(pd.rolling_mean,'mean'),
                    'Индекс мин':rol.rol(pd.rolling_min,'min'),
                    'Индекс макс':rol.rol(pd.rolling_max,'max'),
                    #'Индекс медиана':rol.rol(pd.rolling_median,'median'),
                    #'сред. vol':vol.vol(pd.rolling_mean,'mean'),
                    'vol мин':vol.vol(pd.rolling_min,'min'),
                    'vol макс':vol.vol(pd.rolling_max,'max'),
                    #'vol медиана':vol.vol(pd.rolling_median,'median'),
                    'РЕПО мин': rprp.rprp(pd.rolling_min,'min'),
                    'РЕПО медиана': rprp.rprp(pd.rolling_median,'median'),
                    'РЕПО средняя': rprp.rprp(pd.rolling_mean,'mean'),
                    'РЕПО макс': rprp.rprp(pd.rolling_max,'max')
                    })
    return kv2
...
p=kv2()
p.to_excel('D:/work/data1/Model_mlv_multireg.xlsx')
