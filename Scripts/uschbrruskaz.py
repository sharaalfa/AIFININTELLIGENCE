...
china_fa=ch_fa.unstack().head(43)
kv_3 = np.concatenate(
    [thusa_fa, china_fa*100, brazil_fa,
...
braz_cda=braz[-1:]
china=data_ch[:'       2.2.1.4.1.2 Currency and deposits']
ch_cda=china[-1:]
...
china_cda=ch_cda.unstack().head(43)
...
braz_kza=braz[-1:]
china=data_ch[:'       2.2.1.4.1.3 Loans']
ch_kza=china[-1:]
kv05 = data_052.unstack().head()
kv06 = data_062.unstack().head()
kv07 = data_072.unstack().head()
...
china_kza=ch_kza.unstack().head(43)
kv_7 = np.concatenate(
    [thusa_kza, china_kza*100,
     brazil_kza, kv06, kv07, kv08,
...
braz_tka=braz[-1:]
china=data_ch[:'       2.2.1.4.1.5 Trade credit and advances ']
ch_tka=china[-1:]
kv05 = data_052.unstack().head()
kv06 = data_062.unstack().head()
kv07 = data_072.unstack().head()
...
china_tka=ch_tka.unstack().head(43)
kv_9 = np.concatenate(
    [thusa_tka, china_tka*100, brazil_tka,
     kv06, kv07, kv08,
...
braz_fp=braz[-1:]
china=data_ch[:'         2.2.1.4.2 Liabilities']
ch_fp=china[-1:]
kv05 = data_052.unstack().head()
kv06 = data_062.unstack().head()
kv07 = data_072.unstack().head()
...
china_fp=ch_fp.unstack().head(43)
...
braz_cdp=braz[-1:]
china=data_ch[:'       2.2.1.4.2.2 Currency and deposits']
ch_cdp=china[-1:]
kv051 = data_052.unstack().head()
kv061 = data_062.unstack().head()
kv071 = data_072.unstack().head()
...
china_cdp=ch_cdp.unstack().head(43)
...
braz_kzp=braz[-1:]
usa_kzp=usa[-1:]
china=data_ch[:'       2.2.1.4.2.3 Loans']
ch_kzp=china[-1:]
kv05 = data_052.unstack().head()
kv06 = data_062.unstack().head()
kv07 = data_072.unstack().head()
...
china_kzp=ch_kzp.unstack().head(43)
...
braz_res=braz[-1:]
china=data_ch[:'2.2.2 Reserve assets ']
ch_res=china[-1:]
kv05 = data_052.unstack().head()
kv06 = data_062.unstack().head()
kv07 = data_072.unstack().head()
...
china_res=ch_res.unstack().head(43)
...
braz_tkp=braz[-1:]
china=data_ch[:'       2.2.1.4.2.5 Trade credit and advances ']
ch_tkp=china[-1:]
kv05 = data_052.unstack().head()
kv06 = data_062.unstack().head()
kv07 = data_072.unstack().head()
...
china_tkp=ch_tkp.unstack().head(43)
...
kazah.Close()
Excel5.Quit()
Excel71 = win32com.client.Dispatch("Excel.Application")
kazahin = Excel71.Workbooks.Open(u'D:\work\data\kaz_index.xlsx')
...
bover.Close()
Excel23.Quit()
Excel24=win32com.client.Dispatch("Excel.Application")
shan=Excel24.Workbooks.Open(u'D:\work\data\shan.xlsx')
sheet=shan.ActiveSheet
vals24=[r[0].value for r in sheet.Range("A1:A2636")]
shanchai=pd.DataFrame(vals24)
shan.Close()
Excel24.Quit()
rep_min = z.resample('Q', how='min')
rep_ac_min=z1.resample('Q', how='min')
micx_min = 0.014777*(micex['CLOSE'].resample('Q', how='min')/9.36)
micx_val_min=micex['VALUE'].resample('Q', how='min')
rep = z.resample('Q', how='mean')
rep_ac=z1.resample('Q', how='mean')
micx = 0.014777*(micex['CLOSE'].resample('Q', how='mean')/9.36)
micx_val=micex['VALUE'].resample('Q', how='mean')
rep_max = z.resample('Q', how='max')
rep_ac_max=z1.resample('Q', how='max')
micx_max = 0.014777*(micex['CLOSE'].resample('Q', how='max')/9.36)
micx_val_max=micex['VALUE'].resample('Q', how='max')
c_min=0.125*(pd.rolling_min(shanchai, 60)/1.7)
c_mean=0.125*(pd.rolling_mean(shanchai, 60)/1.7)
c_max=0.125*(pd.rolling_max(shanchai, 60)/1.7)
b_min=0.28*(pd.rolling_min(bovespa, 64)/7.4)
b_mean=0.28*(pd.rolling_mean(bovespa, 64)/7.4)
b_max=0.28*(pd.rolling_max(bovespa, 64)/7.4)
usa_min =  pd.rolling_min(dojo_val, 62)

usa_mean =  pd.rolling_mean(dojo_val, 62)
usa_max = pd.rolling_max(dojo_val, 62)
kaz_min =  0.00365*(pd.rolling_min(kaz_ind, 50)/11.5)

kaz_mean =  0.00365*(pd.rolling_mean(kaz_ind, 50)/11.5)
kaz_max = 0.00365*(pd.rolling_max(kaz_ind, 50)/11.5)
...
kv2 = pd.DataFrame({'Чистое приобретение ф.а.':kv_3,
                    ...
                    'Ошибки и пропуски+резервы': abs(kv_11)+abs(kv_24),
                    'Абс. чоп':abs(kv_23),
                    'Ошибки и пропуски+ссуды+депозиты+резервы':
                        abs(kv_5+kv_7-kv_14-kv_16)+abs(kv_11)+abs(kv_24),
                    'Ошибки и пропуски+ссуды+депозиты':
                        abs(kv_7+kv_5-kv_14-kv_16)+abs(kv_11),
                    'Ошибки и пропуски(без)+ссуды+депозиты+резервы':
                        abs(kv_5+kv_7-kv_14-kv_16)+abs(kv_23)+abs(kv_24),
                    'Ошибки и пропуски(без)+ссуды+депозиты':
                        abs(kv_7+kv_5-kv_14-kv_16)+abs(kv_23),
                    'Ошибки и пропуски(абс)+ссуды-':abs(kv_16)+abs(kv_23),
                    'Ошибки и пропуски(абс)+торгкредиты-':abs(kv_25)+abs(kv_23),
                    'Ошибки и пропуски(абс)+депозиты-':abs(kv_14)+abs(kv_23),
                    'Ошибки и пропуски(абс)+резервы-':abs(kv_24)+abs(kv_23),
                    'Ошибки и пропуски, резервы и все пассивы':
                        abs(kv_23)+abs(kv_16+kv_25+kv_14)+abs(kv_24),
                    'Индекс сред.':ind_un_mean,
                    'Индекс мин.':ind_un_min,
                    'Индекс макс.':ind_un_max})
#kv2.to_excel('D:/work/data/Model_z2.xlsx')
#figure()
#plot(ind_un_max, 'r', label='ind max')
#plot(ind_un_mean, 'b', label='ind mean')
